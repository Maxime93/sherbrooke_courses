...
fiche = "http://www.usherbrooke.ca/fiches-cours/index.php?id=GRH76"
url = fiche
...
if real_title == "Attention" :
    ...
    # Insert to database --------------------
    cur.execute('SELECT Title, Code FROM Courses WHERE Code = ? ', (course_code.string, ))
    row = cur.fetchone()
    if row is None:
